				...
				<div class="col-md-<?php echo esc_attr( $mycoach_page_column ); ?>">
					<div class="page-content">
						...
						while( have_posts() ) : the_post();
							the_content();
							do_action( 'mycoach_story_format_content_after', $post );
							mycoach_link_pages();
							if ( ( $mycoach_options['show-comments-story'] ) && ( comments_open() || '0' != get_comments_number() ) ) {
								comments_template( '', true );
							}
							do_action( 'mycoach_story_item_end' );
						endwhile;
						?>
					</div>
				</div>
				<?php
				if ( $mycoach_options['story-show-previous-next-pagination'] ) {
					// prev and next posts
					$mycoach_prev = get_previous_post( true, null, 'story-category' );
					$mycoach_next = get_next_post( true, null, 'story-category' );
					?>
