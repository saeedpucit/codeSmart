...
(function ($, window, document, undefined) {
    ...
    var $proSearch, $proFixedHeader, $proMainMenu, $proFooter, $proFixedMenu, $proIsDevice, $proUnderDesk, $mycoach_masthead, $mycoach_has_admin_bar, $proSlider;
    NETBEE.vars = function () {
        $proSearch = document.getElementById('sb-search');
        $proMainMenu = $('.main-navigation');
        $proFixedHeader = (mycoach_ajax.sticky) ? true : false;
        $proFooter = $('footer');
        $proSlider = $('.pro-slider');
        $proFixedMenu = $('#header-sticky');
        $mycoach_masthead = $('#masthead');
        $proIsDevice = ( navigator.userAgent.match(/(Android|webOS|BlackBerry|iPod|iPhone|iPad|Opera Mini)/) ) ? true : false;
        $mycoach_has_admin_bar = ( $('body').hasClass('admin-bar') ) ? true : false;
    };
    ...
    NETBEE.fixedHeader = function () {
        if ($proFixedHeader && !$proIsDevice && !mycoach_ajax.sideMenu && !$proUnderDesk) {
    ...
    NETBEE.ajaxPagination = function () {
        $('.ajax-load-more').each(function () {
            var $this = $(this),
                ...
                settings = window['mycoach_load_more_' + token],
    ...
    NETBEE.onePage = function () {
        ...
        if ($('body').hasClass('page-template-page-templatespage-one-page-php')) {
            var _header_height = parseInt($mycoach_masthead.outerHeight(true) + $mycoach_masthead.offset().top),
    ...
    NETBEE.footerEffect = function () {
        if (!$proUnderDesk && mycoach_ajax.showFooter && mycoach_ajax.footerEffect) {
    ...
    var titleParallax = {
        ...
        update: function () {
            var offset = $mycoach_masthead.outerHeight(true),
                factor = 0;
            if ($proSlider) {
